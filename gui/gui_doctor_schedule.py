"""
Модуль с диалогом редактирования индивидуального расписания врача.
"""

import tkinter as tk
from tkinter import ttk, messagebox
import json
from database.appointments import generate_time_slots, get_work_schedule
from database.doctors import get_doctor_schedule, save_doctor_schedule, delete_doctor_schedule

class DoctorScheduleDialog:
    """Диалог редактирования индивидуального расписания врача"""
    
    def __init__(self, parent, doctor_id, doctor_name):
        self.parent = parent
        self.doctor_id = doctor_id
        self.doctor_name = doctor_name
        
        # Загружаем существующее расписание или общее
        self.schedule = get_doctor_schedule(doctor_id)
        self.use_custom = self.schedule is not None
        ...
        self.dialog.title(f"Расписание врача: {doctor_name}")
    ...
    def create_widgets(self):
        ...
        ttk.Label(
            main_frame, 
            text=f"Настройка расписания: {self.doctor_name}", 
            font=('Arial', 14, 'bold')
        ).pack(pady=(0, 15))
        
        # Чекбокс "Использовать индивидуальное расписание"
        self.custom_var = tk.BooleanVar(value=self.use_custom)
        custom_check = ttk.Checkbutton(
            main_frame,
            text="Использовать индивидуальное расписание для этого врача",
            variable=self.custom_var,
            command=self.toggle_custom
        )
        custom_check.pack(anchor='w', pady=10)
        
        # Рамка с настройками
        self.settings_frame = ttk.LabelFrame(main_frame, text="Настройки расписания", padding=10)
        self.settings_frame.pack(fill='x', pady=5)
        
        # Время начала работы
        start_frame = ttk.Frame(self.settings_frame)
        ...
        end_frame = ttk.Frame(self.settings_frame)
        ...
        # Длительность слота
        slot_frame = ttk.Frame(self.settings_frame)
        ...
        lunch_frame = ttk.Frame(self.settings_frame)
        ...
        ttk.Button(
            button_frame, 
            text="Сбросить к общему расписанию", 
    ...
    def toggle_custom(self):
        """Включает/отключает индивидуальное расписание"""
        if self.custom_var.get():
            for child in self.settings_frame.winfo_children():
                if isinstance(child, (ttk.Frame, ttk.Checkbutton, ttk.Spinbox, ttk.Combobox)):
                    for widget in child.winfo_children() if hasattr(child, 'winfo_children') else [child]:
    ...
    def save(self):
        ...
        if not self.custom_var.get():
            # Удаляем индивидуальное расписание
            delete_doctor_schedule(self.doctor_id)
            messagebox.showinfo("Успех", f"Расписание врача {self.doctor_name} сброшено к общему")
            self.dialog.destroy()
            return
        ...
        try:
            ...
            if save_doctor_schedule(self.doctor_id, new_schedule):
                messagebox.showinfo(
                    "Успех", 
                    f"Индивидуальное расписание для врача {self.doctor_name} сохранено!\n\n"
    ...
    def reset_to_default(self):
        """Сбрасывает к общему расписанию"""
        if messagebox.askyesno("Подтверждение", 
                               f"Сбросить расписание врача {self.doctor_name} к общему?\n\n"
                               "Все индивидуальные настройки будут потеряны."):
            delete_doctor_schedule(self.doctor_id)
            messagebox.showinfo("Успех", f"Расписание врача {self.doctor_name} сброшено к общему")
            self.dialog.destroy()
